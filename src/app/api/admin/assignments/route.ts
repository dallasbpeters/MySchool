...
export async function GET() {
  try {
    ...
    const { data: assignmentsData, error: assignmentsError } =
      await supabase.rpc('get_all_assignments_with_parents')
    ...
    const { data: allStudentAssignments, error: studentAssignmentsError } =
      await supabase
    ...
    allStudentAssignments?.forEach(
      (sa: {
        ...
        student_id: string
        profiles: { name: string; role: string; parent_id: string }
      }) => {
