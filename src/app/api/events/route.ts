...
export async function GET() {
  try {
    ...
    if (profile.role === 'admin') {
      ...
      const { data: allAssignments } = await supabase.rpc(
      ...
      if (allAssignments && allAssignments.length > 0) {
        ...
        studentAssignments?.forEach(
          (sa: { assignment_id: string; profiles: { name: string } }) => {
        ...
        (assignment: Assignment & { profiles?: { name: string } }) => {
          ...
          const studentName = (
            assignment as unknown as {
              student_assignments?: { profiles?: { name: string } }
            }
          ).student_assignments?.profiles?.name
          if (
            studentName &&
            !assignmentMap
              .get(assignmentId)
              .assigned_students.includes(studentName)
          ) {
      ...
      completionData?.forEach(
        (completion: {
          ...
          if (completion.completed) {
            assignmentCompletionMap
              .get(key)
              .completed.push(completion.student_id)
          } else {
            assignmentCompletionMap
              .get(key)
              .incomplete.push(completion.student_id)
          }
        },
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    const { title, description, startDate, endDate, color, userId } =
      await request.json()
