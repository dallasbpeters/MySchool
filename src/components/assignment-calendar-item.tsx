...
export function AssignmentCalendarItem({
  ...
  const { displayData, statusText, statusClasses, currentUserStudents } =
    useMemo(() => {
      ...
      const display = formatAssignmentDisplayText(
        assignment,
        maxInitialsDisplay,
      )

      const status =
        assignment.assignedStudents && assignment.completionStatus
          ? getAssignmentStatusText(
      ...
      const getStatusClasses = () => {
        const baseClasses =
          'calendar-assignment border-l-4 p-2 rounded-r-md cursor-pointer transition-all hover:shadow-md'
  ...
  const handleCompletionToggleAction = (
    studentId: string,
    completed: boolean,
  ) => {
    if (onCompletionToggleAction && assignment.assignmentId) {
      onCompletionToggleAction(assignment.assignmentId, completed, studentId)
    }
  }
  ...
  return (
    <div
      ...
      data-completion-status={
        assignment.completionStatus?.allCompleted ? 'complete' : 'incomplete'
      }
